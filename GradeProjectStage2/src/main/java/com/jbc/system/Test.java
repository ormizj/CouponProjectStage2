...
public class Test {
	...
	public void testAll() {
		...
		try {
			...
			adminServ.addCustomer(admin.getId(), new Customer("Yonatan", "Caspi", "Caspi@gmail.com", "0128Moalizko"));
			...
			companyServ.addCoupon(updatedCompany.getId(),
			...
			coupons.stream().forEach(System.out::println);
			...
			companyServ.getCompanyCoupons(updatedCompany.getId()).stream().forEach(System.out::println);
			companyServ.getCompanyCoupons(updatedCompany.getId(), 8).stream().forEach(System.out::println);
			...
			adminServ.getCoupons().stream().forEach(System.out::println);

			/* CustomerService methods */
			customerServ.purchaseCoupon(updatedCustomer.getId(), updatedCoupon.getId());
			customerServ.getCustomerCoupons(updatedCustomer.getId()).stream().forEach(System.out::println);
			customerServ.getCustomerCoupons(updatedCustomer.getId(), 8).stream().forEach(System.out::println);
