...
public interface CouponRepository extends JpaRepository<Coupon, Long> {
	...
	public List<Coupon> findByCustomerIdAndCouponCategory(long customerId, CategoryUtil couponCategory);

	@Query("SELECT coupon FROM Customer customer JOIN customer.coupons coupon WHERE customer.id = ?1 AND coupon.price <= ?2")
	public List<Coupon> findByCustomerIdAndCouponPrice(long customerId, double maxPrice);
