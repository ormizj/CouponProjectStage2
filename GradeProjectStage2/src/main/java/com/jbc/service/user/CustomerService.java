...
public class CustomerService
	...
	public boolean login(String email, String password) {
		...
		loggerRepo.save(new Logger(loggedCustomer.getId(), UserTypeUtil.CUSTOMER, loggedCustomer.getId(),
				ModelEntityUtil.CUSTOMER, ModelActionUtil.LOG_IN, attributeCustomer(loggedCustomer), "-"));
		return true;
	}
	...
	public Coupon purchaseCoupon(long customerId, long couponId) throws CouponOwnedException, CouponNotFoundException,
		...
		synchronized (StringClass.COUPON_ID_SYNC + couponId) {
			...
			loggerRepo.save(new Logger(customerId, UserTypeUtil.CUSTOMER, purchasedCoupon.getId(),
					ModelEntityUtil.COUPON, ModelActionUtil.PURCHASE, "-", attributeCoupon(purchasedCoupon)));
			return couponRepo.save(purchasedCoupon);
		}
	}

	@Override
	public List<Coupon> getCustomerCoupons(long customerId) throws CouponIsEmptyException {
	...
	@Override
	public List<Coupon> getCustomerCoupons(long customerId, double maxPrice) throws CouponNotFoundException {
		List<Coupon> coupons = couponRepo.findByCustomerIdAndCouponPrice(customerId, maxPrice);
		if (coupons.isEmpty())
	...
	@Override
	public List<Coupon> getCustomerCoupons(long customerId, CategoryUtil category) throws CouponNotFoundException {
		List<Coupon> coupons = couponRepo.findByCustomerIdAndCouponCategory(customerId, category);
		coupons = coupons.stream().filter(couponStream -> category.equals(couponStream.getCategory()))
