...
public class DatabaseCreator {
	...
	public static void createDatabase(AdminServiceIfc adminServ, CompanyServiceIfc companyServ,
			CustomerServiceIfc customerServ) throws CompanyNotFoundException {
		try {
			/* creating companies */
			...
			for (int i = 0; i < companies.length; i++) {
				adminServ.createCompany(1, companies[i]);
			}
			...
			for (int i = 0; i < customers.length; i++) {
				adminServ.addCustomer(1, customers[i]);
			}
			...
			for (int i = 0; i < customers.length; i++) {
				adminServ.updateCustomer(1, customers[i]);
			}
