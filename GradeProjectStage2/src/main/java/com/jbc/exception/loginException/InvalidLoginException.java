 ...
 * login attempt in the {@link com.jbc.service.LoginManagerService} {@code Component},
 * {@code login} method.
 * 
 * @author Or Mizrahi
 * @author Shay Yadin
...
public final class InvalidLoginException extends CustomException {
	...
	public InvalidLoginException(String email, boolean emailExists, UserTypeUtil userType) {
		super.errorCode = ExceptionErrorCodeUtil.InvalidLoginException.toDouble();
		this.email = email;
	...
	public String toString() {
		if (userType.equals(UserTypeUtil.ADMIN))
			return "An " + userType
					+ " with the email and/or password doesn't exist, make sure the information is correct.";
		if (!emailExists) {
			return "A " + userType + " with the email " + email
					+ " and password doesn't exist, make sure the email is correct or create a new account.";
		}
		return "The password is incorrect, try entering your password again.";
	}
